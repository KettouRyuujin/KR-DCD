 ...
 * @author hoykj
 */
public class Kuuga_FAR_kick extends AbstractGameEffect {
    ...
    public Kuuga_FAR_kick(AbstractCreature source, AbstractCreature target, int x) {
        this.duration = 3.2F;
    ...
    public void update() {
        if(this.duration == this.startingDuration){
            ...
            Decade.Trickster(98);//中途切换模型
        }

        this.duration -= Gdx.graphics.getDeltaTime();

        if((this.duration < this.startingDuration - 1.62F) && (this.stage == 0)){
            final Decade Decade = (Decade) AbstractDungeon.player;
            Decade.Trickster(99);//中途切换模型
            this.stage ++;
        }
        ...
        if((this.duration < this.startingDuration - 1.72F) && this.stage == 1){
            final Decade Decade = (Decade) AbstractDungeon.player;
            Decade.Trickster(100);//中途切换模型
            this.stage++;
        }
        else if((this.duration < this.startingDuration - 1.82F) && this.stage == 2){
            final Decade Decade = (Decade) AbstractDungeon.player;
            Decade.Trickster(101);//中途切换模型
            this.stage++;
        }
            ...
            if(this.stage == 3){
                ...
                String KUUGA_KICK_ATLAS = "img/char/DCD_Animation/kuuga/FAR/Kuuga_FAR5.atlas";
                String KUUGA_KICK_JSON = "img/char/DCD_Animation/kuuga/FAR/Kuuga_FAR5.json";
            ...
            if(this.source.drawX < this.target.drawX){
                ...
                AbstractDungeon.effectsQueue.add(new Kuuga_FAR_kick2(this.source, this.target , this.damage,this.start.x,this.start.y));
                this.isDone = true;
            }
        }
    }
