...
public class Axel_CrimsonSmash_2A extends AbstractGameEffect {
    ...
    public static String AXEL_JSON1 = "img/char/DCD_Animational/faiz_Axel/Axel_CrimsonSmash_2A.json";

	public Axel_CrimsonSmash_2A(int d) {
		if (this.img == null) {
	...
	public void update() {
		if(Animation) {
			AXEL2 =  new AbstractAnimation(AXEL_ATLAS,AXEL_JSON1, 0.8f, x, y, 120.0F * Settings.scale, 120.0F * Settings.scale, 1.0f, 1.0f);
			Animation = false;
		}
		if(AXEL2!=null) {
			AXEL2.setMovable(false);
		}
		this.duration -= Gdx.graphics.getDeltaTime();
		if (this.duration < 2.0F) {
			if(Start) {
				AbstractAnimation.changeAnimation(AXEL2, FaizAnimationAction.axel_FAR_2A);
				AXEL2.state.setAnimation(0, "2A", false);
	        	Start = false;
			}
		}
